...
def load_cold_data(data_path: str) -> List[Tuple[str, Dict]]:
    ...
    try:
        ...
        logger.info(f"COLD columns: {list(df.columns)}")
        logger.info(f"COLD shape: {df.shape}")
        ...
        logger.error(f"Error loading COLD data: {e}")
        return []
...
def create_unified_dataset(base_path: str) -> Dict[str, List[Tuple[str,
    ...
    cold_splits = {
        ...
        'dev': 'dev_processed.csv',
        'test': 'test_processed.csv'
    }

    for split, filename in cold_splits.items():
        cold_path = base_path / 'data' / 'processed' / 'cold' / filename
        if cold_path.exists():
            cold_data = load_cold_data(str(cold_path))
            datasets[split].extend(cold_data)
            logger.info(f"Added {len(cold_data)} COLD {split} samples")
        else:
    ...
    for dataset_name in sentiment_datasets:
        for split in ['train', 'dev', 'test']:
            ...
            if json_path.exists():
                ...
                # 限制情緒資料數量
                max_sentiment_samples = 1000 if split == 'train' else 200
                if len(sentiment_data) > max_sentiment_samples:
                    sentiment_data = sentiment_data[:max_sentiment_samples]
                datasets[split].extend(sentiment_data)
                logger.info(f"Added {len(sentiment_data)} {"
                    "dataset_name} {split} samples")
            else:
