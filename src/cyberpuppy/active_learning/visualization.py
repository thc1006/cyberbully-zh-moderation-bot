...
class ActiveLearningVisualizer:
    ...
    def plot_learning_curves(self,
        ...
        ax1.plot(annotations, curve_data['f1_macro'], 'o-', label='F1 Macro', linewidth=2, markersize=6)
        ...
        if 'target_f1' in curve_data:
            ax1.axhline(y=curve_data['target_f1'], color='red', linestyle='--',
                       label=f'Target F1: {curve_data["target_f1"]:.2f}')

        # Plot 2: Per-class F1 scores
        if 'f1_toxic' in curve_data and 'f1_severe' in curve_data:
            ax2.plot(annotations, curve_data['f1_toxic'], 'o-', label='F1 Toxic', linewidth=2, markersize=4)
            ax2.plot(annotations, curve_data['f1_severe'], 'o-', label='F1 Severe', linewidth=2, markersize=4)
    ...
    def plot_annotation_efficiency(self,
    ...
    def plot_query_strategy_comparison(self,
        ...
        for i, (strategy_name, curve_data) in enumerate(strategy_results.items()):
            ...
            ax.plot(annotations, f1_scores, 'o-', label=strategy_name,
    ...
    def plot_annotation_distribution(self,
    ...
    def plot_uncertainty_vs_diversity(self,
        ...
        if 'performance_history' in results and len(results['performance_history']) > 1:
            ...
            for i, ann_count in enumerate(results['performance_history']['annotations']):
                record = {
                    ...
                    'metrics': {
                        'f1_macro': results['performance_history']['f1_macro'][i]
                    }
                }
    ...
    def plot_from_checkpoint(self, checkpoint_path: str) -> str:
        ...
        if performance_history and 'f1_toxic' in performance_history[0]['metrics']:
            curve_data['f1_toxic'] = [record['metrics']['f1_toxic'] for record in performance_history]
        if performance_history and 'f1_severe' in performance_history[0]['metrics']:
            curve_data['f1_severe'] = [record['metrics']['f1_severe'] for record in performance_history]

        # Create plots
        plots_dir = os.path.join(self.save_dir, f"checkpoint_plots_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
        os.makedirs(plots_dir, exist_ok=True)

        # Learning curves
        learning_plot = self.plot_learning_curves(
            curve_data,
            title=f"Active Learning Progress (Checkpoint)",
            save_name=os.path.join(plots_dir, "learning_curves.png")
        )

        # Efficiency plot
        if len(performance_history) > 1:
            efficiency_plot = self.plot_annotation_efficiency(
                performance_history,
