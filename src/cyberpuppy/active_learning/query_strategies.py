...
class HybridQueryStrategy(ActiveLearner):
    ...
    def __init__(
        ...
        **strategy_kwargs,
        ...
        if uncertainty_strategy == "entropy":
            ...
            self.uncertainty_sampler = BayesianUncertaintySampling(model, device, n_dropout)
        ...
        if diversity_strategy == "clustering":
            ...
            self.diversity_sampler = ClusteringSampling(model, device, n_clusters, distance_metric)
            ...
            self.diversity_sampler = RepresentativeSampling(model, device, use_pca, pca_components)
        elif diversity_strategy == "hybrid":
            clustering_ratio = strategy_kwargs.get("clustering_ratio", 0.6)
            self.diversity_sampler = DiversityClusteringHybrid(model, device, clustering_ratio)
        else:
    ...
    def select_samples(
        ...
        """
        logger.info(
            f"Hybrid selection: {self.uncertainty_strategy_name} + {self.diversity_strategy_name}"
        )
        ...
        if n_diversity > 0:
            ...
            if len(remaining_indices) > 0:
                ...
                diversity_sub_indices = self.diversity_sampler.select_samples(
                    remaining_dataset, min(n_diversity, len(remaining_indices)), labeled_data
                )
...
class AdaptiveQueryStrategy(HybridQueryStrategy):
    ...
    def __init__(
        ...
        **strategy_kwargs,
        ...
        """
        super().__init__(
            model, device, "entropy", "clustering", initial_uncertainty_ratio, **strategy_kwargs
        )
    ...
    def adapt_ratio(self):
        ...
        if recent_trend < 0.01:  # Performance stagnating
            ...
            self.uncertainty_ratio = max(0.2, self.uncertainty_ratio - self.adaptation_rate)
            logger.info(
                f"Performance stagnating, increasing diversity. "
                f"New ratio: {self.uncertainty_ratio:.2f}"
            )
        elif recent_trend > 0.05:  # Good improvement
            # Maintain or slightly increase uncertainty
            self.uncertainty_ratio = min(0.8, self.uncertainty_ratio + self.adaptation_rate * 0.5)
            logger.info(
                f"Good improvement, adjusting uncertainty. "
                f"New ratio: {self.uncertainty_ratio:.2f}"
            )
...
class MultiStrategyEnsemble(ActiveLearner):
    ...
    def __init__(
        ...
        for config in strategies:
            strategy = HybridQueryStrategy(
                model, device, config["uncertainty"], config["diversity"], config["ratio"]
            )
    ...
    def select_samples(
        ...
        for i, strategy in enumerate(self.strategies):
            selections = strategy.select_samples(unlabeled_data, n_samples, labeled_data)
