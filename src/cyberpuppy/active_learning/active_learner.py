...
class CyberPuppyActiveLearner:
    ...
    def _initialize_query_strategy(self, strategy_name: str, config: Dict[str, Any]):
        ...
        if strategy_name == "hybrid":
            ...
            return MultiStrategyEnsemble(self.model, self.device, strategies, voting)
        else:
    ...
    def evaluate_model(
        self, test_data: Dataset, test_labels: List[int], task: str = "toxicity"
    ) -> Dict[str, float]:
        """
        ...
        report = classification_report(
            test_labels, predictions, target_names=["none", "toxic", "severe"], output_dict=True
        )
    ...
    def check_stopping_criteria(self, current_f1: float) -> bool:
        ...
        if len(self.performance_history) >= 3:
            recent_f1s = [record["metrics"]["f1_macro"] for record in self.performance_history[-3:]]
    ...
    def save_checkpoint(self, iteration: int):
        ...
        # Save as JSON only (more secure than pickle)
        json_path = os.path.join(self.save_dir, f"checkpoint_iter_{iteration}.json")
    ...
    def get_learning_curve_data(self) -> Dict[str, List[float]]:
        ...
        annotations = [record["total_annotations"] for record in self.performance_history]
        f1_scores = [record["metrics"]["f1_macro"] for record in self.performance_history]
        f1_toxic = [record["metrics"]["f1_toxic"] for record in self.performance_history]
        f1_severe = [record["metrics"]["f1_severe"] for record in self.performance_history]
    ...
    def get_annotation_statistics(self) -> Dict[str, Any]:
        ...
        annotations_per_iter = [len(record["annotations"]) for record in self.annotation_history]
    ...
    def get_status_summary(self) -> Dict[str, Any]:
        ...
        current_f1 = (
            self.performance_history[-1]["metrics"]["f1_macro"] if self.performance_history else 0.0
        )
