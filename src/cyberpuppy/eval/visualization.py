...
class ResultVisualizer:
    ...
    def create_comprehensive_dashboard(
        ...
        if "confusion_matrix" in evaluation_results:
            ...
            fig.add_trace(
                go.Heatmap(z=cm, x=labels, y=labels, colorscale="Blues", showscale=True),
                row=1,
                col=2,
            )
        ...
        if "per_class_metrics" in evaluation_results:
            ...
            fig.add_trace(
                go.Bar(
                    x=classes, y=recalls, name="Recall", marker_color=self.color_palette["warning"]
                ),
                row=1,
                col=3,
            )
        ...
        if error_analysis and "statistics" in error_analysis:
            ...
            fig.add_trace(
                go.Pie(labels=error_types, values=error_counts, name="錯誤類型"), row=2, col=2
            )
    ...
    def plot_metrics_comparison(
        self, metrics_dict: Dict[str, Dict[str, float]], save_path: str = None
    ) -> str:
        """繪製指標對比圖"""
        ...
        for bar, value in zip(bars, support_values):
            ...
            ax.text(
                bar.get_x() + bar.get_width() / 2.0,
                height + max(support_values) * 0.01,
                f"{int(value)}",
                ha="center",
                va="bottom",
            )
    ...
    def plot_confidence_distribution(
        ...
        for class_name in set(predictions):
            class_confidences = [
                conf for pred, conf in zip(predictions, confidence_scores) if pred == class_name
            ]
        ...
        correct_confidences = [
            conf
            for pred, true, conf in zip(predictions, true_labels, confidence_scores)
            if pred == true
        ]
        ...
        for i in range(len(bins) - 1):
            mask = (np.array(confidence_scores) >= bins[i]) & (
                np.array(confidence_scores) < bins[i + 1]
            )
            if np.sum(mask) > 0:
    ...
    def plot_error_analysis(
        self, error_analysis_results: Dict[str, Any], save_path: str = None
    ) -> str:
        """繪製錯誤分析圖表"""
    ...
    def plot_robustness_results(
        self, robustness_results: Dict[str, Any], save_path: str = None
    ) -> str:
        """繪製穩健性測試結果"""
        ...
        ax3.plot(
            angles, robustness_scores_closed, "o-", linewidth=2, color=self.color_palette["primary"]
        )
    ...
    def create_interactive_report(
        ...
        html_content = html_template.format(timestamp=timestamp, content=full_content)
