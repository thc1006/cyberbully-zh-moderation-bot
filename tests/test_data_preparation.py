...
from cyberpuppy.data.loader import (MultiTaskDataLoader, TrainingDataset,
                                    create_data_loader)
...
class TestDataPreprocessor:
    ...
    def test_load_cold_dataset(self, mock_read_csv):
        """測試COLD資料集載入"""
        # 模擬CSV資料
        mock_df = pd.DataFrame({"TEXT": ["這是測試文字1", "這是測試文字2"], "label": [1, 0]})
    ...
    def test_full_pipeline_mock(self):
        ...
        try:
            stats = preprocessor.process_complete_pipeline(
                output_dir=str(output_dir), balance_data=False, validate_data=True
            )
