...
from src.cyberpuppy.eval import (ErrorAnalyzer, MetricsCalculator,
                                 ReportGenerator, ResultVisualizer)
from src.cyberpuppy.eval.error_analysis import ErrorCase
...
class TestErrorAnalyzer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.texts = ["這是正常的對話", "你真的很笨", "我要殺了你", "今天天氣很好", "滾開，垃圾"]
    ...
    def test_create_error_cases(self):
        ...
        # 檢查錯誤案例數量（應該有3個錯誤）
        expected_errors = sum(
            1 for true, pred in zip(self.true_labels, self.predicted_labels) if true != pred
        )
    ...
    def test_difficulty_assessment(self):
        ...
        # 高信心錯誤應該是困難的
        difficulty = self.analyzer._assess_difficulty_level("正常文本", "none", "toxic", 0.9)
        self.assertEqual(difficulty, "hard")

        # 低信心錯誤應該是容易的
        difficulty = self.analyzer._assess_difficulty_level("正常文本", "none", "toxic", 0.3)
...
class TestResultVisualizer(unittest.TestCase):
    ...
    def test_plot_confusion_matrix(self, mock_close, mock_savefig):
        ...
        result_path = self.visualizer.plot_confusion_matrix(
            y_true=self.y_true, y_pred=self.y_pred, class_names=["none", "toxic", "severe"]
        )
    ...
    def test_plot_confidence_distribution(self, mock_close, mock_savefig):
        ...
        result_path = self.visualizer.plot_confidence_distribution(
            confidence_scores=self.confidences, predictions=self.y_pred, true_labels=self.y_true
        )
    ...
    def test_metrics_comparison_data_preparation(self):
        ...
        # 這裡我們主要測試數據結構的正確性
        for _class_name, metrics in metrics_dict.items():
            self.assertIn("precision", metrics)
            self.assertIn("recall", metrics)
...
class TestReportGenerator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.evaluation_results = {
            "metrics": {"accuracy": 0.85, "precision": 0.80, "recall": 0.82, "f1": 0.81},
            "per_class_metrics": {
...
class TestIntegration(unittest.TestCase):
    ...
    def test_end_to_end_evaluation_workflow(self):
        ...
        report_files = report_generator.generate_comprehensive_report(
            evaluation_results=evaluation_results, error_analysis=error_results, formats=["json"]
        )
...
class TestPerformance(unittest.TestCase):
    ...
    def test_large_dataset_handling(self):
        ...
        # 檢查處理時間合理（應該在幾秒內）
        self.assertLess(
            processing_time, 5.0, f"處理 {n_samples} 樣本用時過長: {processing_time:.2f}s"
        )
    ...
    def test_memory_usage(self):
        ...
        # 內存增長應該合理（小於100MB）
        self.assertLess(memory_increase, 100, f"內存使用增長過多: {memory_increase:.2f}MB")
