...
class TestDockerServices:
    ...
    def docker_compose_project(self, docker_client):
        ...
        yield {"project_name": project_name, "compose_file": compose_file}
        ...
        while time.time() - start_time < max_wait:
            try:
                async with httpx.AsyncClient() as client:
                    response = await client.get("http://localho" "st:8000/healthz", timeout=5.0)
    ...
    async def test_bot_container_health(self, docker_compose_project):
        ...
        while time.time() - start_time < max_wait:
            try:
                async with httpx.AsyncClient() as client:
                    response = await client.get("http://localho" "st:8080/health", timeout=5.0)
                    if response.status_code == 200:
                        health_data = response.json()
    ...
    async def test_service_communication(self, docker_compose_project):
        ...
        async with httpx.AsyncClient() as client:
            api_response = await client.post(
                "http://localhost:8000/analyze", json={"text": "Docker 整合測試"}, timeout=30.0
            )
            ...
            bot_response = await client.get("http://localho" "st:8080/health", timeout=10.0)
    ...
    def test_container_logs(self, docker_compose_project):
        ...
        if result.returncode == 0:
            ...
            has_expected_content = any(pattern in logs for pattern in expected_patterns)
    ...
    async def test_container_resource_usage(self, docker_compose_project):
        ...
        # 檢查容器資源使用
        result = subprocess.run(
            ["doc" "ker", "table {{.Container}}\t{{.CPUPerc}}\t{{.MemUsage}}\t{{.MemPerc}}"],
            capture_output=True,
            text=True,
        )
...
class TestDockerNetworking:
    ...
    def test_port_mapping(self, docker_compose_project):
        """測試埠口映射"""
        docker_compose_project["compose_file"]
...
class TestDockerCleanup:
    ...
    def test_remove_orphaned_containers(self):
        """清理孤立容器"""
        result = subprocess.run(
            ["docker", "container", "prune", "-f"], capture_output=True, text=True
        )
...
def get_container_ip(container_name: str) -> str:
    """取得容器 IP 位址"""
    result = subprocess.run(["doc" "ker", container_name], capture_output=True, text=True)
