...
class TestBullyingDetector:
    ...
    def config(self):
        ...
        config.BATCH_SIZE = 8
        config.NUM_CLASSES = {"toxicity": 3, "bullying": 3, "role": 4, "emotion": 3}
        return config
...
class TestEdgeCases(TestBullyingDetector):
    ...
    def test_empty_text_handling(self, config, mock_model, mock_tokenizer):
        ...
        with (
            ...
            assert all(
                key in predictions[0]
                for key in ["toxicity", "bullying", "role", "emotion", "emotion_strength"]
            )
...
class TestEvaluationMetrics(TestBullyingDetector):
    ...
    def test_comprehensive_evaluation(
        self, config, mock_model, mock_tokenizer, sample_texts, sample_labels
    ):
        ...
        with (
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                with (
                    patch.object(evaluator, "metrics_calculator") as mock_metrics,
    ...
    def test_model_comparison(self, config, mock_model, mock_tokenizer):
        ...
        with (
            ...
            baseline_results = {
                "basic_metrics": {"toxicity": {"f1_macro": 0.75}, "bullying": {"f1_macro": 0.70}}
            }

            current_results = {
                "basic_metrics": {"toxicity": {"f1_macro": 0.85}, "bullying": {"f1_macro": 0.75}}
            }

            with tempfile.TemporaryDirectory() as temp_dir:
                comparison = evaluator.compare_models(baseline_results, current_results, temp_dir)
...
class TestIntegration(TestBullyingDetector):
    ...
    def test_full_evaluation_pipeline(self, config, sample_texts, sample_labels):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            results = evaluator.evaluate_comprehensive(sample_texts, sample_labels, temp_dir)
