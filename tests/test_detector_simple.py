...
from cyberpuppy.models.result import (BullyingResult, BullyingType,
                                      ConfidenceThresholds, DetectionResult,
                                      EmotionResult, EmotionType,
                                      ExplanationResult, ModelPrediction,
                                      ResultAggregator, RoleResult, RoleType,
...
def sample_role_result():
    ...
    return RoleResult(
        ...
        confidence=0.65,
        raw_scores={"none": 0.2, "perpetrator": 0.1, "victim": 0.65, "bystander": 0.05},
        threshold_met=True,
    )
...
def sample_explanation_result():
    ...
    return ExplanationResult(
        ...
        tokens=["這", "個", "人", "很", "討厭"],
        explanation_text="Word '很' and '討厭' contribut" "e most to toxicity detection",
...
class TestDetectionResult:
    ...
    def test_detection_result_high_risk_assessment(self, sample_detection_result):
...
class TestResultAggregator:
    ...
    def test_filter_results_by_confidence(self, sample_detection_result):
        ...
        filtered = ResultAggregator.filter_results_by_confidence(results, "toxicity", 0.9)
        assert len(filtered) == 0

        # Filter by toxicity confidence >= 0.8 (should return all 3)
        filtered = ResultAggregator.filter_results_by_confidence(results, "toxicity", 0.8)
...
class TestConfidenceThresholds:
    ...
    def test_update_thresholds(self):
        ...
        original_threshold = ConfidenceThresholds.get_threshold("toxicity", "toxic")

        new_thresholds = {"toxicity": {"toxic": 0.9}}
        ConfidenceThresholds.update_thresholds(new_thresholds)

        updated_threshold = ConfidenceThresholds.get_threshold("toxicity", "toxic")
    ...
    def test_validate_thresholds(self):
        ...
        valid_thresholds = {"toxicity": {"none": 0.5, "toxic": 0.7, "severe": 0.8}}
        assert ConfidenceThresholds.validate_thresholds(valid_thresholds) is True

        invalid_thresholds = {"toxicity": {"none": 0.5, "toxic": 1.5}}  # Invalid: > 1.0
...
class TestCyberPuppyDetectorSimple:
    ...
    def test_detector_ensemble_weights_normalization(self, detector_config):
        ...
        with patch("src.cyberpuppy.models.detector.CyberPuppyDetector._load_models"):
            ...
            detector = CyberPuppyDetector(unnormalized_config, load_models=False)
    ...
    def test_detector_confidence_calibration(self, detector_config):
        ...
        with patch("src.cyberpuppy.models.detector.CyberPuppyDetector._load_models"):
            ...
            high_conf = detector._calibrate_confidence(high_conf_pred, "toxicity")

            # Low confidence prediction
            low_conf_pred = torch.tensor([0.4, 0.35, 0.25])
            low_conf = detector._calibrate_confidence(low_conf_pred, "toxicity")
    ...
    def test_detector_context_manager(self, detector_config):
        ...
        with patch("src.cyberpuppy.models.detector.CyberPuppyDetector._load_models"):
            ...
            with CyberPuppyDetector(detector_config, load_models=False) as detector:
