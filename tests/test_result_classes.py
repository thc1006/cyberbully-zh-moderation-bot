...
class TestResultClasses:
    ...
    def test_complete_detection_result_workflow(self):
        ...
        toxicity = ToxicityResult(
            ToxicityLevel.TOXIC, 0.8, {"none": 0.1, "toxic": 0.8, "severe": 0.1}, True
        )
        emotion = EmotionResult(
            EmotionType.NEGATIVE, 0.75, 3, {"pos": 0.1, "neu": 0.15, "neg": 0.75}, True
        )
        bullying = BullyingResult(
            BullyingType.HARASSMENT, 0.7, {"none": 0.2, "harassment": 0.7, "threat": 0.1}, True
        )
        role = RoleResult(
            RoleType.VICTIM,
            0.65,
            {"none": 0.2, "perpetrator": 0.1, "victim": 0.65, "bystander": 0.05},
            True,
        )
    ...
    def test_result_aggregator_comprehensive(self):
        ...
        high_conf_results = ResultAggregator.filter_results_by_confidence(results, "toxicity", 0.8)
    ...
    def test_confidence_thresholds_management(self):
        ...
        ConfidenceThresholds.update_thresholds({"toxicity": {"toxic": 0.9}})

        assert ConfidenceThresholds.get_threshold("toxicity", "toxic") == 0.9

        # Restore original
        ConfidenceThresholds.update_thresholds({"toxicity": {"toxic": original}})

        # Test validation
        valid_thresholds = {"toxicity": {"none": 0.5, "toxic": 0.7}}
        assert ConfidenceThresholds.validate_thresholds(valid_thresholds) is True

        invalid_thresholds = {"toxicity": {"none": 1.5}}  # Invalid: > 1.0
    ...
    def test_detection_result_validation(self):
        ...
        with pytest.raises(ValueError):
            DetectionResult(
                ...
                timestamp=datetime.now(),
                processing_time=0.1,
                toxicity=ToxicityResult(ToxicityLevel.NONE, 1.5, {}, True),  # Invalid confidence
                emotion=EmotionResult(EmotionType.NEUTRAL, 0.8, 0, {}, True),
                bullying=BullyingResult(BullyingType.NONE, 0.8, {}, True),
                role=RoleResult(RoleType.NONE, 0.8, {}, True),
                explanations={},
    ...
    def test_model_prediction_serialization(self):
        ...
        pred = ModelPrediction(
            ...
            predictions={"task1": torch.tensor([0.2, 0.8]), "task2": torch.tensor([0.1, 0.4, 0.5])},
        ...
        assert torch.allclose(restored.predictions["task1"], torch.tensor([0.2, 0.8]))
    ...
    def test_edge_cases_and_error_handling(self):
        ...
        filtered = ResultAggregator.filter_results_by_confidence([result], "unknown_task", 0.5)
        assert filtered == []
